...
import { FaShoppingCart, FaUser, FaCalendar, FaWarehouse, FaBox, FaDollarSign, FaPlus, FaTrash, FaEdit, FaSave, FaTimes, FaEye, FaCopy, FaFileAlt, FaSearch, FaFilter, FaUndo } from 'react-icons/fa';
...
const Page = () => {
  ...
  const [goodsReturn, setGoodsReturn] = useState({
    returnNumber: '',
    customer: '',
    customerName: '',
    contactPerson: '',
  ...
  // Sample existing returns for view mode
  ...
  const calculateTotals = (items = orderItems) => {
    ...
    const freight = goodsReturn.freight || 0;
    const tax = goodsReturn.tax || 0;
  ...
  const handleAddItem = () => {
    ...
    addToast('New item added to return', 'success');
  };
  ...
  const handleRemoveItem = (index) => {
    if (orderItems.length > 1) {
      ...
      setOrderItems(newItems);
      calculateTotals(newItems);
      addToast('Item removed from return', 'info');
    } else {
  ...
  const handleSubmit = (action) => {
    if (goodsReturn.customer && orderItems.some(item => item.description && item.quantity > 0)) {
      // Here you would typically make an API call to save the goods return
      console.log('Saving goods return:', { goodsReturn, orderItems });
      
      if (action === 'addAndClose') {
        addToast('Goods return created successfully!', 'success');
        setTimeout(() => {
          navigate('/goods-return');
        }, 1500);
      } else if (action === 'addAndView') {
        addToast('Goods return created successfully!', 'success');
        // Stay on the page to view the created return
      }
    } else {
  ...
  const switchToViewMode = () => {
    setViewMode('view');
    addToast('Switched to view mode - viewing existing returns', 'info');
  };

  // Switch to create mode
  const switchToCreateMode = () => {
    setViewMode('create');
    addToast('Switched to create mode - creating new return', 'info');
  };

  // Render view mode (existing returns)
  if (viewMode === 'view') {
    return (
      <Layout>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-800">Goods Returns</h1>
                  <p className="text-gray-600">View and manage existing goods returns</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <button
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-1 relative">
                <input
                  type="text"
                  placeholder="Search returns by number, customer, or status..."
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <tbody className="divide-y divide-gray-100">
                  {existingReturns.map((returnItem, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <span className="text-sm font-medium text-gray-900">{returnItem.customer}</span>
                          <p className="text-xs text-gray-500">{returnItem.contactPerson}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full border ${
                          returnItem.status === 'Open' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                          returnItem.status === 'Processed' ? 'bg-green-100 text-green-800 border-green-200' :
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-900">{returnItem.postingDate}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-900">{returnItem.returnReason}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-medium text-gray-900">${returnItem.grandTotal.toFixed(2)}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => addToast(`Viewing return ${returnItem.returnNumber}`, 'info')}
                          ...
                          <button
                            onClick={() => addToast(`Editing return ${returnItem.returnNumber}`, 'info')}
                          ...
                          <button
                            onClick={() => addToast(`Return ${returnItem.returnNumber} deleted successfully`, 'success')}
            ...
            {existingReturns.length === 0 && (
              <div className="text-center py-12">
                <FaUndo className="w-12 h-12 text-gray-300 mx-auto mb-4" />
  ...
  // Render create mode (new return form)
  return (
    <Layout>
      <div className="p-6 space-y-6">
        {/* Header Section */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Goods Return</h1>
                <p className="text-gray-600">Create and manage goods returns</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              <button
              ...
              <button
                onClick={() => addToast('New return functionality', 'info')}
                className="px-3 py-1.5 text-white rounded-md transition-colors duration-200 flex items-center space-x-2 hover:shadow-md transform hover:-translate-y-0.5 text-sm"
                style={{ backgroundColor: primaryColor }}
              >
                <FaPlus className="w-3.5 h-3.5" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={goodsReturn.customerName}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={goodsReturn.contactPerson}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Customer Reference No</label>
                <input
                  type="text"
                  value={goodsReturn.customerRefNo}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={goodsReturn.returnNumber}
                  onChange={(e) => handleInputChange('returnNumber', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={goodsReturn.documentDate}
              ...
              <div>
                ...
                <select
                  value={goodsReturn.returnReason}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={goodsReturn.salesEmployee}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">${goodsReturn.totalAmount.toFixed(2)}</span>
                </div>
                ...
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span>${goodsReturn.grandTotal.toFixed(2)}</span>
                </div>
              </div>
            </div>
