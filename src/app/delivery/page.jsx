...
import { FaTruck, FaUser, FaPlus, FaEye, FaEdit, FaTrash, FaSave, FaTimes, FaBox } from 'react-icons/fa';
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600">View and manage deliveries</p>
                </div>
              </div>
              <button
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Delivery Number</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Customer</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Status</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Delivery Date</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {existingDeliveries.map((delivery, index) => (
                    <tr key={delivery.id} className={`hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <td className="px-6 py-4"><span className="text-sm font-medium text-gray-900">{delivery.deliveryNumber}</span></td>
                      <td className="px-6 py-4"><span className="text-sm text-gray-900">{delivery.customer}</span></td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full border ${
                          delivery.status === 'In Transit' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                      ...
                      <td className="px-6 py-4"><span className="text-sm text-gray-900">{delivery.deliveryDate}</span></td>
                      <td className="px-6 py-4"><span className="text-sm text-gray-900">{delivery.totalItems}</span></td>
                      <td className="px-6 py-4"><span className="text-sm font-medium text-gray-900">${delivery.totalAmount.toFixed(2)}</span></td>
                      <td className="px-6 py-4">
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-600">Create and manage deliveries</p>
              </div>
            </div>
            <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-800 flex items-center">
                <FaTruck className="w-5 h-5 mr-2" style={{ color: primaryColor }} />
          ...
          <div className="space-y-4">
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              <p className="text-gray-500 text-center">Delivery items will be added here</p>
            </div>
          </div>
